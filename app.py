...
def is_gpu_available():
    if not GPU_AVAILABLE:
        logging.warning("GPU support not available: GPUtil module not found")
        return {"gpu_found": False, "num_gpus": 0, "first_gpu_vram": 0, "total_vram": 0, "error": "GPUtil module not found"}
    try:
        gpus = GPUtil.getGPUs()
        num_gpus = len(gpus)
        if num_gpus == 0:
        ...
        first_gpu_vram = gpus[0].memoryTotal
        total_vram = sum(gpu.memoryTotal for gpu in gpus)
        logging.info(f"GPU(s) found: {num_gpus}, Total VRAM: {total_vram} MB")
        return {"gpu_found": True, "num_gpus": num_gpus, "first_gpu_vram": first_gpu_vram, "total_vram": total_vram, "gpu_info": gpus}
...
class App:
    def __init__(self, root):
        self.root = root
        self.root.title("OCR and Text Processing")
        
        self.input_file_label = tk.Label(root, text="Input PDF File:")
        ...
        self.output_file_label = tk.Label(root, text="Output DOCX File:")
        ...
        self.log_text = scrolledtext.ScrolledText(root, width=100, height=20)
        self.log_text.pack()
    ...
    def browse_output_file(self):
        filename = filedialog.asksaveasfilename(defaultextension=".docx", filetypes=[("DOCX files", "*.docx")])
...
async def process_chunk(chunk: str, prev_context: str, chunk_index: int, total_chunks: int, reformat_as_markdown: bool, suppress_headers_and_page_numbers: bool) -> Tuple[str, str]:
    ...
    ocr_correction_prompt = f"""You are an advanced language model tasked with correcting text that has been converted from an image using Optical Character Recognition (OCR). The OCR process may have introduced errors such as incorrect words, misspellings, and grammatical mistakes. Your goal is to review the text and make the necessary corrections to ensure it is accurate and well-written. Follow these guidelines:

1. Fix OCR-induced typos and errors:
   - Correct words split across line breaks
   - Fix common OCR errors (e.g., 'rn' misread as 'm')
   - Correct any spelling mistakes.
   - Replace incorrect words with the correct ones based on context.
   - Do not make any changes to the overall document except for clear errors introduced by the OCR software.
   - When a sentence does not make sense, replace the word that makes the sentence incorrect with the correct word.
   - Do not make arbitrary changes just to improve the overall sound of the document; only correct errors that are clearly obvious
...
3. Preserve original content:
   - Keep all information from the original text except for the necessary corrections  
...
async def main(input_pdf_file_path, output_docx_file_path, log_text):
    try:
        ...
        if USE_LOCAL_LLM:
            ...
            log_text.insert(tk.END, f"Model download status: {download_status}\n")
            log_text.insert(tk.END, f"Using Local LLM with Model: {DEFAULT_LOCAL_MODEL_NAME}\n")
        else:
            log_text.insert(tk.END, f"Using API for completions: {API_PROVIDER}\n")
            log_text.insert(tk.END, f"Using OpenAI model for embeddings: {OPENAI_EMBEDDING_MODEL}\n")
        
        list_of_scanned_images = convert_pdf_to_images(input_pdf_file_path, max_test_pages, skip_first_n_pages)
        log_text.insert(tk.END, f"Tesseract version: {pytesseract.get_tesseract_version()}\n")
        log_text.insert(tk.END, "Extracting text from converted pages...\n")
        log_text.update()
        
        with ThreadPoolExecutor() as executor:
            list_of_extracted_text_strings = list(executor.map(ocr_image, list_of_scanned_images))
        
        log_text.insert(tk.END, "Done extracting text from converted pages.\n")
        log_text.update()
        
        raw_ocr_output = "\n".join(list_of_extracted_text_strings)
        log_text.insert(tk.END, "Processing document...\n")
        log_text.update()
        ...
        # Save the LLM corrected output in DOCX format
        doc = Document()
        doc.add_paragraph(cleaned_text)
        doc.save(output_docx_file_path)
        
        log_text.insert(tk.END, f"LLM Corrected text written to: {output_docx_file_path}\n")
        log_text.update()
        
        if final_text:
            log_text.insert(tk.END, f"First 500 characters of LLM corrected processed text:\n{final_text[:500]}...\n")
        else:
            log_text.insert(tk.END, "final_text is empty or not defined.\n")
        
        log_text.insert(tk.END, f"Done processing {input_pdf_file_path}.\n")
        log_text.insert(tk.END, "\nSee output files:\n")
        log_text.insert(tk.END, f" LLM Corrected: {output_docx_file_path}\n")
        log_text.update()
        
        quality_score, explanation = await assess_output_quality(raw_ocr_output, final_text)
        if quality_score is not None:
            log_text.insert(tk.END, f"Final quality score: {quality_score}/100\n")
            log_text.insert(tk.END, f"Explanation: {explanation}\n")
        else:
            log_text.insert(tk.END, "Unable to determine final quality score.\n")
        log_text.update()
    except Exception as e:
        log_text.insert(tk.END, f"An error occurred in the main function: {e}\n")
